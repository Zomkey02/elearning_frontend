      ...
      <div className="w-full">
        ...
        {open && (
          <div className='pb-4 space-y-2 lg:hidden'>
            <Link to='/elearning' className='link-nav' onClick={closeMenu}>E-Learning</Link>
            <Link to='/blog' className='link-nav' onClick={closeMenu}>Blog</Link>
            <Link to='/about' className='link-nav' onClick={closeMenu}>About</Link>
            
            <Link to='/contact' className='link-nav' onClick={closeMenu}>Contact</Link>

            {isLoggedIn && (
              <Link to='/dashboard' className='link-nav' onClick={closeMenu}>Dashboard</Link>
            )}

            {isLoggedIn ? ( <LogOut isVisible={true} /> ) : (
              <>
                <Link to='/login' className='link-nav' onClick={closeMenu}>Login</Link>
                <Link to='/signup' className='btn-nav' onClick={closeMenu}>Sign up</Link>
              </>
            )}       
          </div>
        )}
